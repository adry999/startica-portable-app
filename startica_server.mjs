...
export function createApplication(options = {}) {
  ...
  if (!existed && db.prepare("SELECT name FROM sqlite_master WHERE type='table' AND name='app_state'").get())
  ...
  if (
    !db.prepare("SELECT value FROM settings WHERE key='legacyMigrated'").get() &&
    db.prepare("SELECT name FROM sqlite_master WHERE type='table' AND name='app_state'").get()
  ) {
  ...
  function selectedBackup(name) {
    if (typeof name !== 'string' || basename(name) !== name || !/^startica_[A-Za-z0-9_.-]+\.db$/.test(name))
  ...
  function commit(body, action, fn, preBackup = false) {
    if (typeof body.requestId !== 'string' || !/^[a-zA-Z0-9-]{10,100}$/.test(body.requestId))
    ...
    db.exec('BEGIN IMMEDIATE');
    try {
      if (body.revision !== db.prepare('SELECT revision FROM meta').get().revision)
        fail('Date modificate în altă filă.', 409);
      fn();
  ...
  function send(res, value, status = 200, mime = 'application/json; charset=utf-8') {
    ...
    res.writeHead(status, {
      ...
      'Content-Security-Policy':
        "default-src 'self'; script-src 'self'; style-src 'self' 'unsafe-inline' https://fonts.googleapis.com; font-src 'self' https://fonts.gstatic.com; connect-src 'self'; object-src 'none'; base-uri 'none'; frame-ancestors 'none'",
    });
    res.end(content);
  }
  async function readJson(req) {
    ...
    for await (const c of req) {
      size += c.length;
      if (size > 20000000) fail('Fișierul este prea mare.', 413);
      chunks.push(c);
    }
  ...
  const server = createServer(async (req, res) => {
    try {
      ...
      if (req.method === 'GET' && staticFiles[path])
        return send(
          res,
          readFileSync(join(root, staticFiles[path])),
          200,
          path.endsWith('.svg')
            ? 'image/svg+xml'
            : path.endsWith('.css')
      ...
      if (req.method === 'GET' && path === '/api/backup-preview') {
        const s = snapshotState(selectedBackup(url.searchParams.get('name')));
        return send(res, { ...summary(s), errors: importReport(s).errors });
      }
      if (req.method === 'GET' && path === '/api/audit') {
        const offset = Math.max(0, Number(url.searchParams.get('offset')) || 0);
        return send(res, db.prepare('SELECT * FROM audit_changes ORDER BY id DESC LIMIT 100 OFFSET ?').all(offset));
      }
      ...
      if (path === '/api/record')
        return send(
          ...
          commit(b, 'salvare', () => {
            ...
            ).run(b.type, r.id, JSON.stringify(r));
            audit(old ? 'modificare' : 'adăugare', b.type, r.id, old, r);
          }),
        );
      if (path === '/api/import-preview') return send(res, importReport(b.state));
      if (path === '/api/financial-preview') {
            ...
            () => {
              ...
              for (const type of ['payments', 'expenses'])
                for (const r of plan.additions[type]) {
                  db.prepare('INSERT INTO records VALUES(?,?,?)').run(type, r.id, JSON.stringify(r));
                  audit('import istoric V5', type, r.id, null, r);
                }
            },
            true,
          ),
      ...
      if (path === '/api/children-csv-preview') {
        const current = envelope();
        return send(res, { ...previewChildrenCSV(b.csv, current.state.children), revision: current.revision });
      }
          ...
          commit(
            ...
            () => {
              ...
              for (const r of report.additions) {
                db.prepare('INSERT INTO records VALUES(?,?,?)').run('children', r.id, JSON.stringify(r));
                audit('import copii CSV', 'children', r.id, null, r);
              }
            },
            true,
          ),
      ...
      if (path === '/api/import') {
        ...
        return send(
          res,
          commit(b, 'import', () => replace(s, 'import'), true),
        );
      }
      if (path === '/api/restore') {
        if (b.confirm !== 'RESTAUREAZA') fail('Confirmă restaurarea.');
        const s = validateState(snapshotState(selectedBackup(b.name)));
        return send(
          res,
          commit(b, 'restaurare', () => replace(s, 'restaurare'), true),
        );
      }
      ...
      if (path === '/api/settings') {
        ...
        if (folder) {
          ...
          for (const reserved of [dataDir, backupDir])
            if (
              absolute === resolve(reserved) ||
              (!relative(resolve(reserved), absolute).startsWith('..') &&
                !isAbsolute(relative(resolve(reserved), absolute)))
            )
        ...
        audit('configurare backup', null, null, { externalDir: before }, { externalDir: folder });
        return send(res, { ok: true, ...safeBackup('configurare'), health: health() });
      }
