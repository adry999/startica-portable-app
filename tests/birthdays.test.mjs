...
import { monthCalendar, upcomingBirthdays } from '../shared/domain.mjs';
...
test('monthCalendar aliniază grila luni-duminică și umple din lunile vecine', () => {
  // Septembrie 2026 începe marți (2026-09-01), deci luni 31 august e umplutură.
  const weeks = monthCalendar([], '2026-09-10');
...
test('monthCalendar marchează ziua curentă și copiii cu ziua de naștere în lună', () => {
  const children = [
    child('c1', '2019-09-20'),
    child('c2', '2018-08-15'), // altă lună, nu apare
    child('c3', '2020-09-10', { archived: true }), // arhivat, nu apare
  ];
  const weeks = monthCalendar(children, '2026-09-10');
...
test('monthCalendar marchează săptămâna curentă, chiar dacă traversează granița de lună', () => {
...
test('monthCalendar potrivește ziua de naștere după lună+zi, nu după an', () => {
  const weeks = monthCalendar([child('old', '1990-09-05')], '2026-09-10');
  const cell = cellFor(weeks, '2026-09-05');
  assert.deepEqual(cell.names, [{ name: 'old', turningAge: 2026 - 1990 }]);
});

test('upcomingBirthdays ia doar zilele din fereastra dată, sortate crescător', () => {
  const children = [
  ...
  const rows = upcomingBirthdays(children, 5, '2026-09-10');
  assert.deepEqual(
...
test('upcomingBirthdays traversează corect granița de lună/an', () => {
  const rows = upcomingBirthdays([child('ian', '2015-01-02')], 5, '2025-12-30');
