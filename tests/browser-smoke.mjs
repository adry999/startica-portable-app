...
const chrome=spawn(executable,['--headless=new','--disable-gpu','--no-first-run','--no-default-browser-check','--remote-debugging-port=0',`--user-data-dir=${join(dir,'profile')}`,'--window-size=1440,1000','about:blank'],{windowsHide:true,stdio:'ignore'});
let ws, seq=0;const pending=new Map(),errors=[];
const sleep=ms=>new Promise(r=>setTimeout(r,ms));
async function until(fn,message){for(let n=0;n<100;n++){if(await fn())return;await sleep(100);}throw Error(message);}
try{
  ...
  const target=await(await fetch(`http://127.0.0.1:${port}/json/new?${encodeURIComponent(url)}`,{method:'PUT',signal:AbortSignal.timeout(5000)})).json();
  ws=new WebSocket(target.webSocketDebuggerUrl);await new Promise((r,j)=>{const timer=setTimeout(()=>j(Error('WebSocket timeout')),5000);ws.onopen=()=>{clearTimeout(timer);r()};ws.onerror=j});
  ws.onmessage=e=>{const m=JSON.parse(e.data);if(m.method==='Runtime.exceptionThrown')errors.push(m.params.exceptionDetails.text+' '+JSON.stringify(m.params.exceptionDetails.exception));if(m.id){const p=pending.get(m.id);pending.delete(m.id);m.error?p.reject(m.error):p.resolve(m.result)}};
  ws.onclose=e=>console.error('Browser connection closed: '+e.code+' '+e.reason);
  const command=(method,params={})=>new Promise((resolve,reject)=>{const id=++seq;const timer=setTimeout(()=>reject(Error('Timeout: '+method)),5000);pending.set(id,{resolve:value=>{clearTimeout(timer);resolve(value)},reject});ws.send(JSON.stringify({id,method,params}));});
  ...
  await until(()=>evaluate("document.getElementById('saveIndicator')?.dataset.state==='saved'"),'Application failed to load');
  ...
  await evaluate("document.querySelector('[data-create=children]').click();document.getElementById('editorForm').elements.name.dispatchEvent(new Event('input',{bubbles:true}))");
  ...
  await until(()=>evaluate("document.getElementById('saveIndicator').dataset.state==='saved'"),'Cancel should clear draft status');
  await evaluate("document.querySelector('[data-create=children]').click()");
  assert.equal(await evaluate("(()=>{const f=document.getElementById('editorForm');f.elements.name.value='Test';f.elements.name.dispatchEvent(new Event('input',{bubbles:true}));const e=new Event('beforeunload',{cancelable:true});window.dispatchEvent(e);return e.defaultPrevented;})()"),true);
  ...
  await evaluate("window.testFetch=window.fetch;window.fetch=async(...args)=>{if(args[0]==='/api/record'){const response=await window.testFetch(...args);return new Promise((resolve,reject)=>{window.failSave=()=>reject(new TypeError('Test lost response'));});}return window.testFetch(...args);}");
  await evaluate("(()=>{const f=document.getElementById('editorForm');f.elements.name.value='Copil <test>';f.elements.parent.value='Părinte test';f.elements.parent2.value='Al doilea părinte';f.elements.phone2.value='060123456';f.elements.group.value='1';f.elements.attendanceDate.value='2026-09-01';f.elements.fee.value='2000';f.elements.feeFrom.value='2026-09';f.elements.statusFrom.value='2026-09';f.requestSubmit();})()");
  ...
  await until(()=>evaluate("document.getElementById('saveIndicator').dataset.state==='error'"),'Lost response must show red');
  await evaluate("document.querySelector('[data-close=editor]').click()");
  ...
  await until(()=>evaluate("document.getElementById('saveIndicator').dataset.state==='saved'"),'Retry must confirm saved status');
  ...
  await evaluate("(()=>{const f=document.getElementById('editorForm');f.elements.status.value='Retras';f.elements.statusFrom.value='2026-10';f.elements.withdrawalDate.value='2026-09-30';f.requestSubmit();})()");
  ...
  await evaluate("(()=>{const f=document.getElementById('editorForm');f.elements.phone.value='123';f.requestSubmit();})()");
  ...
  await evaluate("document.querySelector('[data-close=profile]').click();document.querySelector('[data-view=review]').click()");
  assert.match(await evaluate("document.getElementById('reviewList').textContent"),/Avans nerepartizat/);
  await evaluate("document.querySelector('[data-view=settings]').click();document.getElementById('restoreButton').click()");
  await until(()=>evaluate("document.getElementById('restorePreview').textContent.includes('3000') || document.getElementById('restorePreview').textContent.includes('3.000') || document.getElementById('restorePreview').textContent.includes('3 000')"),'Restore preview failed');
  await evaluate("document.querySelector('[data-close=restoreDialog]').click();document.querySelector('[data-view=audit]').click()");
  await until(()=>evaluate("document.getElementById('auditList').textContent.includes('modificare')"),'Audit UI failed');
  await evaluate("document.querySelector('[data-view=settings]').click();const dir=document.getElementById('externalDir');dir.value='C:\\\\does-not-exist-startica-test';dir.dispatchEvent(new Event('input',{bubbles:true}));window.dispatchEvent(new Event('focus'))");
  assert.equal(await evaluate("document.getElementById('saveIndicator').dataset.state"),'pending');
  await sleep(300);
  ...
  await until(()=>evaluate("document.getElementById('saveIndicator').dataset.state==='error'"),'Invalid settings must show red');
  await evaluate("document.getElementById('externalDir').value='';document.getElementById('externalDir').dispatchEvent(new Event('input',{bubbles:true}));document.getElementById('settingsForm').requestSubmit()");
  await until(()=>evaluate("document.getElementById('saveIndicator').dataset.state==='saved'"),'Settings retry must show green');
  await evaluate("window.fetch=async()=>{throw new TypeError('Test offline')};window.dispatchEvent(new Event('focus'))");
  await until(()=>evaluate("document.getElementById('saveIndicator').dataset.state==='error'"),'Offline server must show red');
  await evaluate("window.fetch=window.testFetch;document.getElementById('reloadButton').click()");
  await until(()=>evaluate("document.getElementById('saveIndicator').dataset.state==='saved'"),'Connection recovery must show green');
  const csv='ID (Nr. contract),Nume copil,Parinte,Telefon,Data nasterii,Data frecventarii\n888,CSV <test>,Parinte CSV,060999999,01.01.2022,01.09.2026';
  ...
  await evaluate("document.getElementById('csvConfirm').value='IMPORT COPII';document.getElementById('csvConfirm').dispatchEvent(new Event('input',{bubbles:true}));document.getElementById('commitCsv').click()");
  ...
  assert.equal(imported.children.length,2);assert.equal(imported.payments.length,1);assert.equal(imported.payments[0].amount,3000);
  await evaluate(chooseCsv);await until(()=>evaluate("document.getElementById('csvDialog').open"),'CSV reimport preview failed');
  ...
  console.log('PASS: header saved/draft/saving/error states, cancel, lost-response retry without duplicates, settings preservation/retry, offline/reconnect; load, child, XSS, payment allocations, dashboard, profile, review, restore preview, audit.');
}catch(error){console.error(error);process.exitCode=1;}finally{
  ws?.close();chrome.kill();app.server.closeAllConnections();await app.close();await sleep(1200);
  if(resolve(dir).startsWith(resolve(tmpdir())+'\\startica-browser-')||resolve(dir).startsWith(resolve(tmpdir())+'/startica-browser-')){try{rmSync(dir,{recursive:true,force:true});}catch{console.log('Temporary test folder retained: '+dir);}}
}
