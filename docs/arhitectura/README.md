...
Pasul 11 a împărțit `web/app.css` (2132 linii) în `web/styles/{base,layout,components,print}.css` și `web/styles/features/{backup,billing,children,dashboard,data-transfer,expenses,groups,payments,record-editing,review-center}.css`, câte un fișier per feature care deține reguli proprii; `index.html` leagă un `<link>` per fișier, în ordinea bază → layout → componente → feature-uri (alfabetic) → print, care înlocuiește cascada dintr-un singur fișier. `STATIC_FILES` din `static-assets.mjs` capătă o intrare explicită per fișier nou; un test nou verifică că fiecare `<link rel="stylesheet">` din `index.html` e în lista albă și fișierul mapat există pe disc. Selectoarele duplicate aflate departe unele de altele în fișierul original (`.topbar`, `.panel`, `.alert`, `.alerts`, `.save-indicator`, `.toolbar`, `.top-actions`, `.brand`, `.cumulative-summary small`) au rămas ca reguli separate, în ordinea relativă originală, în loc să fie combinate — cascada rezultată e identică, iar combinarea nu era necesară. Câteva reguli dintr-un fișier cu prioritate mai mică pierdeau, în ordinea nouă, în fața unei reguli cu aceeași specificitate dintr-un fișier cu prioritate mai mare care în fișierul original venea *după* ea (`.cumulative-summary strong` vs. `.card strong`, `.cumulative-summary small`/`p` vs. `.card small`/`p`, `.field input` vs. regula generică `input`, `.field.full` responsive vs. `.form-grid .full`, `.group-create` vs. `.toolbar`): partea din regulă dovedită de oracol ca fiind mereu suprascrisă în original a fost eliminată din fișierul cu prioritate mai mică, nu adăugată sau mutată. O singură diferență a rămas ireductibilă: `.attention-panel`/`.birthdays-panel` (`dashboard.css`) au aceeași specificitate ca `.panel` (`layout.css`), iar `print.css` — care trebuie încărcat ultimul — le suprascrie padding-ul (22px → 0) în ecranul Dashboard la tipărire; în original cele două reguli veneau, întâmplător, după blocul `@media print`, deci câștigau. Fără regulă nouă sau reordonare de fișiere nu există o soluție conformă; diferența e invizibilă în aplicația reală, pentru că `print.css` ascunde orice ecran în afară de Situația plăților și De notificat, singurele din care se tipărește vreodată. Verificare: oracol de stiluri calculate (`getComputedStyle` pentru fiecare element, inclusiv pseudo-elementele `::before`/`::after`/`::marker`/`::placeholder`/`::backdrop`, pe o copie a bazei reale) — 87 de scenarii (12 ecrane × 5 lățimi, print per ecran, 8 stări deschise la 1440/400px), 144 048 de perechi element/pseudo-element comparate (toate proprietățile `getComputedStyle`, per pereche), 2 diferențe reziduale (cele de mai sus); auto-verificare bază-vs-bază (același cod de două ori) = 0 diferențe, verificare cu o modificare plantată (`.btn { padding: 1px }`) = detectată.
