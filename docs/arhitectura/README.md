...
Pasul 12 a extins testul de granițe (`tests/architecture/import-boundaries.test.mjs`) de la `src/` la tot codul livrat: `scripts/`, `tests/` (fără fixture-urile proprii) și fișierele de la rădăcină, fiecare cu regulile lui (scripturile și testele nu ating internele unui feature, doar `index.server.mjs`/`index.web.mjs`). `scripts/import-v5-history.mjs` citește acum din API-ul public al `data-transfer`, nu din `domain/` și `server/` direct. Fațadele enumerate în rândul 12 fuseseră deja șterse la remedierea auditului din 14 septembrie. `tests/e2e/*` din plan e, în practică, `npm run test:e2e` (browser smoke + `desktop-lifecycle.ps1`), iar pachetul de livrare se construiește cu `scripts/pachet-client/build-client-package.ps1`.
...
Regulile sunt verificate automat de `tests/architecture/import-boundaries.test.mjs`. Testul parsează specificatorii din `src/**/*.mjs`, `scripts/**/*.mjs`, `tests/**/*.mjs` (fără fixture-urile din `tests/architecture/`) și fișierele de la rădăcină, și rulează în `npm test`.
