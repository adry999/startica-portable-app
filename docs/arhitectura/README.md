...
| 4 | Core server: `util` → `errors`; `http` → `core/server/http` (dispatcher cu listă de rute); `database` + `migrations` → `core/server/database`; `store` → `record-repository` + `revision-transaction`; `createApplication` → `src/app/server` cu aceeași semnătură | Schema, migrările (conținut identic), idempotența, revizia, backupul înainte | **Mediu**: tranzacții și migrări pe baze reale | `application`, `fixes`, `groups`; test de pornire pe o copie a `startica_2026-09-10_livrare.db` într-un director temporar |
...
| Actual | Țintă | Pas |
| --- | --- | --- |
| `startica_server.mjs` | shim → `src/app/server/main.mjs` + `create-application.mjs` | 4 |
