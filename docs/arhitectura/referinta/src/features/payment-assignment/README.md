...
│   ├── unassigned-payment-queue.mjs         # listUnassignedPayments  (azi unassignedPayments)
│   ├── unassigned-payment-risk.mjs          # measureAssignmentRisk   (azi assignmentRisk)
│   ├── unassigned-payment-hints.mjs         # findUnassignedPaymentHintsByChild (azi unassignedSuggestionsByChild)
...
În `docs/arhitectura/referinta` există fișierele marcate ★, testele, API-ul, rutele și tipurile. Fișierele din `domain/` și view-ul se mută la pasul 7.
